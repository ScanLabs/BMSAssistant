#VERSION 5.0 - Needs ScanLabs dongle version 3.3.0
import global
import haspmota
import string
import json
...
def calcPos( StrIn, offset)
  var calcX=0
  calcX += size(StrIn) * 12
  calcX += string.count(StrIn, '0') * 1
  calcX += string.count(StrIn, '8') * 1
...
#  No json or NO dashboard page (Pd) ? Malformed Json -> exit
  ...
  var Vtx = 30 #global.(Pg + "b18").x
  var Ax = 20 #global.(Pg + "b28").x
  var Wx = 25 #global.(Pg + "b38").x
  var Cpx = 30 #global.(Pg + "b58").x
  var Avx = 252 #global.(Pg + "b78").x
  var Dlx = 256 #global.(Pg + "b88").x
  var Tx = 256 #global.(Pg + "b98").x
  ...
  if (index == 1 && SOC>=0 ) 
    ...
    drawline(int(payload_json.find("SM")), 2, 60, 70, 75, 75)
